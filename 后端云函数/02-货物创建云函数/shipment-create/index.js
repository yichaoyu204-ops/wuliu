...
async function driverSubmit(event) {
  ...
  if (!cargoInfo || !contacts || !contacts.contactPhone) {
  ...
  try {
    ...
    if (waybillNo.trim()) {
      ...
      if (dupRes.data.length > 0) {
        return error(`运单号 ${waybillNo.trim()} 已存在，请勿重复建单`);
      }
    }
    ...
    // 格式化工厂名
    const factoryName = contacts.factoryName
      ? (contacts.factoryName.startsWith('萌恒-') ? contacts.factoryName : `萌恒-${contacts.factoryName}`)
      : '未知工厂';

    // 构建货物文档 - 所有配置都保存在文档中，后续修改config不会影响此文档
    const shipmentDoc = {
      _id: shipmentId,
      managerPhone: contacts.contactPhone,
      clientType,
      clientName: contacts.contactPerson || '未知',
      factoryName,
    ...
    // 检查用户绑定状态
    const userRes = await db.collection('users').where({ phone: contacts.contactPhone }).get();
